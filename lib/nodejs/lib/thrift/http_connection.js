 ...
 */
HttpConnection.prototype.write = function(data, seqid, _extraOptions) {
  var self = this;
  var opts = self.nodeOptions;
  opts.headers["Content-length"] = data.length;
  if (!opts.headers["Content-Type"])
    opts.headers["Content-Type"] = "application/x-thrift";
  if (_extraOptions && _extraOptions.headers && typeof _extraOptions.headers === 'object') {
    var extraHeaderKeys = Object.keys(_extraOptions.headers);
    for (var i = 0; i < extraHeaderKeys.length; i++) {
      var key = extraHeaderKeys[i];
      opts.headers[key] = _extraOptions.headers[key];
    }
  }
  var req = (self.https) ?
