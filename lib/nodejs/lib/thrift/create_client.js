...
function createClient(ServiceClient, connection) {
  ...
  // wrap the handlers functions to handle extraParams
  const wrapped = {};
  for (const funcName in client) {
    if (typeof client[funcName] !== 'function') continue;
    wrapped[funcName] = function() {
      let parsedArguments = Array.from(arguments);
      const lastArg = arguments[arguments.length - 1];
      const definedNumArgs = client[funcName].length - 1; // subtract 1 because the generated code defines the callback arg
      if (typeof lastArg !== 'function' && arguments.length > definedNumArgs) {
        client.extraParams = parsedArguments.pop();
      }
      return client[funcName](...parsedArguments);
    }
  }
  return wrapped;
};
