...
/**
 * @description Wraps a function with a number of arguments passed in as `length`
 * Commentary: This is mainly to address the fact that Thrift uses the
 * handler functions' length property. So our wrapper functions must
 * contain the original number of arguments.
 * Source: http://stackoverflow.com/a/7317057
 *
 * @method generateFingerprint
 * @param {Number}   length  number of arguments to (max = 10)
 * @param {Function} fn      the function to wrap
 * @return {Function}
 */
const createVariableArgumentFunc = (length, fn) => {
  /* eslint-disable brace-style, max-statements-per-line, id-length, comma-spacing */
  if (length > 10) throw new Error('too many arguments');
  switch (length) {
  ...
  /* eslint-enable brace-style, max-statements-per-line, id-length, comma-spacing */
};
...
exports.createWebServer = function(options) {
  ...
  for (var uri in services) {
    ...
    for (const funcName in svcObj.handler) {
      if (typeof svcObj.handler[funcName] !== 'function') continue;
      const func = svcObj.handler[funcName];
      svcObj.handler[funcName] = createVariableArgumentFunc(func.length, function() {
        const headers = { extraHeaders : svcObj.processor.headers };
        if (headers) {
          return func(...arguments, headers);
        }
  ...
  function processPost(request, response) {
    ...
    svc.processor.headers = request.headers;
